# به‌روزرسانی زنده سامانه گرایه (نسخه ۱۲.۲.۱)
...
```powershell
Compress-Archive -Path app,web,deploy,VERSION,requirements.txt -DestinationPath garaye-12.2.1.zip
```
...
```powershell
deploy\apply-update.ps1 -ZipPath C:\path\garaye-12.2.1.zip
```
