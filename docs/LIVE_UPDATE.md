# به‌روزرسانی زنده سامانه گرایه (نسخه ۱۲.۱.۰)
...
```powershell
Compress-Archive -Path app,web,deploy,VERSION,requirements.txt -DestinationPath garaye-12.1.0.zip
```
...
```powershell
deploy\apply-update.ps1 -ZipPath C:\path\garaye-12.1.0.zip
```
