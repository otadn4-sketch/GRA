# به‌روزرسانی زنده سامانه گرایه (نسخه ۱۱.۳۴.۰)
...
```powershell
Compress-Archive -Path app,web,deploy,VERSION,requirements.txt -DestinationPath garaye-11.34.0.zip
```
...
```powershell
deploy\apply-update.ps1 -ZipPath C:\path\garaye-11.34.0.zip
```
