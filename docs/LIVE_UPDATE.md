# به‌روزرسانی زنده سامانه گرایه (نسخه ۱۱.۳۳.۰)
...
```powershell
Compress-Archive -Path app,web,deploy,VERSION,requirements.txt -DestinationPath garaye-11.33.0.zip
```
...
```powershell
deploy\apply-update.ps1 -ZipPath C:\path\garaye-11.33.0.zip
```
